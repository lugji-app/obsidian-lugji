...
export default class LugjiPlugin extends Plugin {
	...
	async onload(): Promise<void> {
		...
		this.addRibbonIcon(
			"mic",
			// eslint-disable-next-line obsidianmd/ui/sentence-case -- "Lugji" is the brand name; ribbon tooltip needs it for context (no auto plugin-name prefix on ribbon icons).
			"Start Lugji recording",
	...
	private openUrl(url: string): void {
		...
		if (!opened) {
			new Notice(
				// eslint-disable-next-line obsidianmd/ui/sentence-case -- proper noun "Lugji" + acronym "iOS" are intentional; rephrasing harms UX clarity.
				"Could not open the Lugji iOS app. Make sure it is installed on this device.",
			);
		}
	}
